...
class TransportModel_entropy(nn.Module):
    ...
    def dtransport_dS(self, alpha, entropy):
        fun = vmap(val_and_jacrev(self.__call__, argnums=1, has_aux=False),
                   in_axes=(0, 0), out_axes=0)
        dtransport_dS, transport = fun(alpha, entropy)
        dtransport_dS = dtransport_dS.reshape(entropy.shape)
        transport = transport.reshape(entropy.shape)
        return transport, dtransport_dS
