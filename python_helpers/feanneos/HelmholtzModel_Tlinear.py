...
class HelmholtzModel_Tlinear(nn.Module):
    """
    ...
    def __call__(self, alpha, rhoad, Tad):
        ...
        x_rhoad0 = jnp.stack([alpha, rhoad0, Tad]).T
    ...
    def d3helmholtz_dT3(self, alpha, rhoad, Tad):
        fun = vmap(val_and_jacrev(val_and_jacrev(val_and_jacrev(
                   self.__call__, argnums=2, has_aux=False),
                   argnums=2, has_aux=True),
                   argnums=2, has_aux=True), in_axes=(0, 0, 0))
        d3Ares_dT3, (d2Ares_dT2, (dAres_dT, Ares)) = fun(alpha, rhoad, Tad)
        d3Ares_dT3 = d3Ares_dT3.reshape(rhoad.shape)
        d2Ares_dT2 = d2Ares_dT2.reshape(rhoad.shape)
        dAres_dT = dAres_dT.reshape(rhoad.shape)
        Ares = Ares.reshape(rhoad.shape)
        return Ares, dAres_dT, d2Ares_dT2, d3Ares_dT3

    def d3helmholtz(self, alpha, rhoad, Tad):
        fun = vmap(val_and_jacrev(val_and_jacrev(val_and_jacrev(
                   self.__call__, argnums=[1, 2], has_aux=False),
                   argnums=[1, 2], has_aux=True),
                   argnums=[1, 2], has_aux=True), in_axes=(0, 0, 0))
        out = fun(alpha, rhoad, Tad)

        ((d3Ares_drho2_dT, d3Ares_drho_dT2), (d3Ares_drho_dT2, d3Ares_dT3)) = out[0][1]
        ((d3Ares_drho3, d3Ares_drho2_dT), (d3Ares_drho2_dT, d3Ares_drho_dT2)) = out[0][0]
        ((d2Ares_drho2, d2Ares_drho_dT), (d2Ares_drho_dT, d2Ares_dT2)) = out[1][0]
        ((dAres_drho, dAres_dT), Ares) = out[1][1]

        d3Ares_drho3 = d3Ares_drho3.reshape(rhoad.shape)
        d3Ares_drho2_dT = d3Ares_drho2_dT.reshape(rhoad.shape)
        d3Ares_drho_dT2 = d3Ares_drho_dT2.reshape(rhoad.shape)
        d3Ares_dT3 = d3Ares_dT3.reshape(rhoad.shape)
        ...
        outd3 = Ares, (dAres_drho, dAres_dT), (d2Ares_drho2, d2Ares_drho_dT, d2Ares_dT2), (d3Ares_drho3, d3Ares_drho2_dT, d3Ares_drho_dT2, d3Ares_dT3)
        return outd3
    ...
    def second_virial_coefficient(self, params, alpha, Tad):
        # check that the shapes are correct
        assert alpha.shape == Tad.shape
        rhoad = jnp.zeros_like(Tad)

        Ares, dAres_drho = self.apply(params, alpha, rhoad, Tad, method=self.dhelmholtz_drho)
        B2 = dAres_drho / Tad
        return B2

    def third_virial_coefficient(self, params, alpha, Tad):
    ...
    def virial_coefficients(self, params, alpha, Tad):
        ...
        Ares, dAres_drho, d2Ares_drho2, d2Ares_drho_dT = out
        B2 = dAres_drho / Tad
        dB2_dT = - dAres_drho / Tad**2 + d2Ares_drho_dT / Tad
        B3 = d2Ares_drho2 / Tad
        return B2, dB2_dT, B3
